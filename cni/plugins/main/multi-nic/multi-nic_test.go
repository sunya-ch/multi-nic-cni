...
func generateConfigTest(conf, masterName string) {
...
var _ = Describe("Operations", func() {
	...
	for _, ver := range []string{"0.3.0"} {
		...
		It(fmt.Sprintf("[%s] configures and deconfigures link with ADD/DEL (multi-nic IPAM)", ver), func() {
			...
			conf := genIPVLANL3Config(ver, multiNICIPAM, masterNets)
		...
		It(fmt.Sprintf("[%s] configures and deconfigures link with ADD/DEL (single-nic IPAM)", ver), func() {
			conf := genIPVLANL3Config(ver, singleNICIPAM, masterNets)
			multinicAddCheckDelTest(conf, "", originalNS, targetNS)
		})
	}

	It("generate other plugin configs with host local ipam", func() {
		masterNetsBytes, _ := json.Marshal(POOL_NETWORK_ADDRESSES)
		masterNets := string(masterNetsBytes)
		_ = originalNS.Do(func(ns.NetNS) error {
			// sriov
			pluginValue := fmt.Sprintf(`{
				"cniVersion":  "0.3.0",
				"type": "sriov",
				"deviceID": ""
			}`)
...
func genIPVLANL3Config(ver, ipamValue, masterNets string) string {
	return fmt.Sprintf(`{ 
		"cniVersion": "0.3.0", 
...
func genConfigWithHostLocal(mainPluginValue, masterNets string) ([]byte, *NetConf) {
	confStr := fmt.Sprintf(`{ 
		"cniVersion": "0.3.0", 
		"name": "multi-nic-sample",
		"type": "multi-nic",
		...
		"subnet": "192.168.0.0/16",
		"masterNets": %s
		}`, mainPluginValue, BRIDGE_HOST_IP, daemonPort, masterNets)
	conf := []byte(confStr)
	n := &NetConf{}
